...
function App() {
  ...
  const sendMessage = async () => {
    ...
    const isCheckoutIntent = checkoutKeywords.some((keyword) =>
    ...
    if (isCheckoutIntent && !isRegistered) {
      const errorMessage: Message = {
        ...
        content:
          "To complete checkout, please register first. Click the 'Register' button in the header to create your account with secure passkey authentication.",
        role: "assistant",
        timestamp: new Date(),
      };
    ...
    try {
      ...
      if (response.data.products && Array.isArray(response.data.products)) {
        ...
        assistantMessage.products?.forEach((p) => {
          ...
          if (
            response.data.response.toLowerCase().includes("in cart") ||
  ...
  const handleResetDatabase = async () => {
    if (
      !confirm(
        "⚠️ WARNING: This will delete ALL user data, payment cards, and transactions from the database. This action cannot be undone. Are you sure you want to continue?",
      )
    ) {
      return;
    }
    ...
    try {
      ...
      const errorMessage: Message = {
        id: Date.now().toString(),
        content:
          "❌ Failed to reset database. Please try again or contact support.",
        role: "assistant",
        timestamp: new Date(),
      };
      setMessages((prev) => [...prev, errorMessage]);
  ...
  const handleAddToCartFromGrid = async (
    productId: string,
    productName: string,
  ) => {
        ...
        <div className="max-w-6xl mx-auto space-y-6">
          ...
          {messages.map((message) => (
            ...
            >
              ...
              {/* Message Bubble */}
              <div
                className={`flex-1 ${message.role === "user" ? "items-end" : "items-start"} flex flex-col`}
              >
                <div
